...
from verl.c1_trainer.method.ray_trainer_v2 import RayPPOTrainer
...
# 将一个普通函数变成一个“可配置的主函数”。
# hydra 会自动解析命令行参数、加载 config/ppo_trainer.yaml 的内容、把所有配置构造成一个 结构化的 config 对象、传入 main() 函数
...
class TaskRunner:
    ...
    def run(self, config):
        ...
        if config.exp.setting == "run_ours":
            ...
            from verl.c1_trainer.method.ray_trainer_v2 import ResourcePoolManager, Role   # 一定要导入自定义的Role，不然会匹配不上
        ...
        # Load the reward manager for training and validation. # config 里要指定自定义的 reward_manager
