...
from verl.utils.reward_score import default_compute_score, custom_compute_score
...
def get_custom_reward_fn(config):
    ...
    spec = importlib.util.spec_from_file_location("custom_module", file_path)  # 创建一个虚拟模块名 "custom_module"，把文件内容加载进去。
    module = importlib.util.module_from_spec(spec)   # 调用 spec.loader.exec_module(module) 执行该 Python 文件，使其定义的函数/类生效。
    try:
