...
class AutoScoringJudge:
    ...
    def extract_and_judge(self, expression1, expression2, precision=1e-8):
        # Judge if two expressions are equal (expression1 is considered as the Ground Truth)
        # Default precision is a list for supporting multiple expressions
        precision = precision if isinstance(precision, list) else [precision]
        ...
        if expression1 == expression2:
            # print("Exactly equal")
            return True, expression1
        
        # Remove Chinese characters from the string, as answers like "yes" or "no" in Chinese have been considered
        expression1_res = expression1
        expression1 = re.sub(r'[\u4e00-\u9fff]+', '', expression1)
        expression2 = re.sub(r'[\u4e00-\u9fff]+', '', expression2)
...
olympiad_scorer = AutoScoringJudge()
def verify(solution_str, ground_truth, precision):
    true_or_false, extracted_pred = olympiad_scorer.extract_and_judge(solution_str, ground_truth, precision)
    return true_or_false, extracted_pred

def compute_score(
    solution_str: str,
    ground_truth: str,
    extra_info: dict,  # {"precision": [1e-8,]}
    ...
    # Verify the solution
    assert "precision" in extra_info, "please check your data_process script to make sure 'precision' is one of the sample attribute"
    correct, pred = verify(solution_str, ground_truth, extra_info["precision"])
